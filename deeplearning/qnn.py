...
class BinaryLeNet_5(StoQNN):
    def __init__(self, in_dims, in_channels, out_dims=10):
        super(BinaryLeNet_5, self).__init__()
        self.in_channels = in_channels
        self.input_size = int(np.sqrt(in_dims/in_channels))
        self.fc_input_size = int(self.input_size/4)**2 * 64
        
        self.conv1 = BinaryConv2d(self.in_channels, 32, kernel_size=5, padding=2)
        self.bn1 = nn.BatchNorm2d(32, track_running_stats=False, affine=False)
        self.mp1= nn.MaxPool2d(kernel_size=2, stride=2)
        
        self.conv2 = BinaryConv2d(32, 64, kernel_size=5, padding=2)
        self.bn2 = nn.BatchNorm2d(64, track_running_stats=False, affine=False)
        self.mp2= nn.MaxPool2d(kernel_size=2, stride=2)

        self.fc1 = BinaryLinear(self.fc_input_size, 512)
        self.bn3 = nn.BatchNorm1d(512, track_running_stats=False, affine=False)
        self.fc2 = nn.Linear(512, out_dims)
...
class BinaryVGG_7(StoQNN):
    def __init__(self, in_dims, in_channels, out_dims=10):
        super(BinaryVGG_7, self).__init__()
        self.in_channels = in_channels
        self.input_size = int(np.sqrt(in_dims/in_channels))
        self.fc_input_size = int(self.input_size/8)**2 * 512
        
        self.conv1_1 = BinaryConv2d(self.in_channels, 128, kernel_size=3, padding=1)
        ...
        self.fc1 = BinaryLinear(self.fc_input_size, 1024)
...
class BinaryResNet_8(StoQNN):
    def __init__(self, in_dims, in_channels, out_dims=10):
        super(BinaryResNet_8, self).__init__()
        self.in_channels = in_channels
        self.input_size = int(np.sqrt(in_dims/in_channels))
        self.avg_pool_input_size = int(self.input_size/8)

        self.in_planes = 128
        self.conv1 = BinaryConv2d(in_channels, 128, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(128, track_running_stats=False, affine=False)
        self.mp1 = nn.MaxPool2d(kernel_size=2, stride=2)

        self.layer1 = self._make_layer(BinaryBasicBlock, 128, 1, stride=1)
        self.layer2 = self._make_layer(BinaryBasicBlock, 256, 1, stride=2)
        self.layer3 = self._make_layer(BinaryBasicBlock, 512, 1, stride=2)
